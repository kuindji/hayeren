...
/** Drops undefined values and empty arrays. Never pass a required array through it (see `cases` below). */
...
it("a typed question without a matching questionGroups entry still shows up, in a nameless group of its type", () => {
  const files2: DataFiles = { ...files, cases: [{ ...files.cases[1]!, questionGroups: undefined, questions: [{ type: "pronoun", question: { russian: "чей" } }, { type: "noun", question: { russian: "чего" } }] }] };
  const c = new Database(files2).case.get("possessive")!;
