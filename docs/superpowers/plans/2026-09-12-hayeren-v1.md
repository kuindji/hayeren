...
it("reports a noun in a declension group that lacks that declension in that case (invisible on the site)", () => {
  ...
  d.words.noun[0]!.cases[0]!.declension = undefined as never; // drop the declension: still a valid noun file
...
export function checkReferences(d: DataFiles): string[] {
  ...
  for (const c of d.cases) {
    ...
    for (const g of c.groups ?? []) for (const w of g.words) if (!nounIds.has(w)) problems.push(`${where}: unknown noun "${w}" in group`);
...
import {
  ...
  type DataFiles, type ArticleFile, type WordType,
} from "./schema";
import { parseFrontmatter } from "./frontmatter";
